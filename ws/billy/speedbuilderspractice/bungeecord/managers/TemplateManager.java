...
public class TemplateManager
{
            ...
            for (int j = min2; j <= max2; ++j) {
                for (int k = min3; k <= max3; ++k) {
                    if (j != min2) {
                        ...
                        if (!state.getType().toString().equals("AIR") && state.getBlockData().getAsString().equals(this.plugin.getBungeeCord().currentBuildBlocks.get(n))) {
            ...
            for (int j = min; j <= max; ++j) {
                for (int k = min2; k <= max2; ++k) {
                    for (int l = min3; l <= max3; ++l) {
                        ...
                        state.setBlockData(Bukkit.createBlockData(this.plugin.getConfigManager().getConfig("templates.yml").getString("templates." + this.plugin.getBungeeCord().currentBuildRawName + ".blocks." + i)));
            ...
            for (int n2 = min4; n2 <= max4; ++n2) {
                for (int n3 = min5; n3 <= max5; ++n3) {
                    for (int n4 = min6; n4 <= max6; ++n4) {
                        final BlockState state2 = Bukkit.getWorld(this.plugin.getBungeeCord().currentMap).getBlockAt(n2, n3, n4).getState();
                        state2.setBlockData(Bukkit.createBlockData(this.plugin.getConfigManager().getConfig("templates.yml").getString("templates." + this.plugin.getBungeeCord().currentBuildRawName + ".blocks." + n)));
    ...
    public void unloadTemplate(final String str) {
        if (str.equalsIgnoreCase("guardian")) {
            for (final String str2 : this.plugin.getConfigManager().getConfig("maps.yml").getConfigurationSection("maps." + this.plugin.getBungeeCord().currentMap + ".plots.guardian.blocks").getKeys(false)) {
                ...
                state.setBlockData(Bukkit.createBlockData(this.plugin.getConfigManager().getConfig("maps.yml").getString("maps." + this.plugin.getBungeeCord().currentMap + ".plots.guardian.blocks." + str2)));
                state.update(true, false);
            }
        }
        else {
    ...
    public void resetPlots(final String str) {
        if (this.plugin.getConfigManager().getConfig("maps.yml").contains("maps." + str + ".plots")) {
            for (final String str2 : this.plugin.getConfigManager().getConfig("maps.yml").getConfigurationSection("maps." + str + ".plots").getKeys(false)) {
                if (this.plugin.getConfigManager().getConfig("maps.yml").contains("maps." + str + ".plots." + str2 + ".blocks")) {
                    for (final String str3 : this.plugin.getConfigManager().getConfig("maps.yml").getConfigurationSection("maps." + str + ".plots." + str2 + ".blocks").getKeys(false)) {
                        final String[] split = str3.split(",");
                        final BlockState state = Bukkit.getWorld(str).getBlockAt(Integer.parseInt(split[0]), Integer.parseInt(split[1]), Integer.parseInt(split[2])).getState();
                        state.setBlockData(Bukkit.createBlockData(this.plugin.getConfigManager().getConfig("maps.yml").getString("maps." + str + ".plots." + str2 + ".blocks." + str3)));
                        state.update(true, false);
                    }
                }
            }
    ...
    public void savePlots(final String s) {
        for (final String s2 : this.plugin.getConfigManager().getConfig("maps.yml").getConfigurationSection("maps." + s + ".plots").getKeys(false)) {
            this.plugin.getConfigManager().getConfig("maps.yml").set("maps." + s + ".plots." + s2 + ".blocks", (Object)"");
            if (s2.equalsIgnoreCase("guardian")) {
                ...
                for (int i = min; i <= max; ++i) {
                    for (int j = min2; j <= max2; ++j) {
                        for (int k = min3; k <= max3; ++k) {
                            this.plugin.getConfigManager().getConfig("maps.yml").set("maps." + s + ".plots.guardian.blocks." + i + "," + j + "," + k, (Object)Bukkit.getWorld(s).getBlockAt(i, j, k).getState().getBlockData().getAsString().replaceFirst("minecraft:", ""));
                        }
                    }
                }
            }
            else {
                final int min4 = Math.min(this.plugin.getConfigManager().getConfig("maps.yml").getInt("maps." + s + ".plots." + s2 + ".area.x1"), this.plugin.getConfigManager().getConfig("maps.yml").getInt("maps." + s + ".plots." + s2 + ".area.x2"));
                final int min5 = Math.min(this.plugin.getConfigManager().getConfig("maps.yml").getInt("maps." + s + ".plots." + s2 + ".area.y1"), this.plugin.getConfigManager().getConfig("maps.yml").getInt("maps." + s + ".plots." + s2 + ".area.y2"));
                final int min6 = Math.min(this.plugin.getConfigManager().getConfig("maps.yml").getInt("maps." + s + ".plots." + s2 + ".area.z1"), this.plugin.getConfigManager().getConfig("maps.yml").getInt("maps." + s + ".plots." + s2 + ".area.z2"));
                final int max4 = Math.max(this.plugin.getConfigManager().getConfig("maps.yml").getInt("maps." + s + ".plots." + s2 + ".area.x1"), this.plugin.getConfigManager().getConfig("maps.yml").getInt("maps." + s + ".plots." + s2 + ".area.x2"));
                final int max5 = Math.max(this.plugin.getConfigManager().getConfig("maps.yml").getInt("maps." + s + ".plots." + s2 + ".area.y1"), this.plugin.getConfigManager().getConfig("maps.yml").getInt("maps." + s + ".plots." + s2 + ".area.y2"));
                final int max6 = Math.max(this.plugin.getConfigManager().getConfig("maps.yml").getInt("maps." + s + ".plots." + s2 + ".area.z1"), this.plugin.getConfigManager().getConfig("maps.yml").getInt("maps." + s + ".plots." + s2 + ".area.z2"));
                for (int l = min4; l <= max4; ++l) {
                    for (int m = min5; m <= max5; ++m) {
                        for (int i2 = min6; i2 <= max6; ++i2) {
                            this.plugin.getConfigManager().getConfig("maps.yml").set("maps." + s + ".plots." + s2 + ".blocks." + l + "," + m + "," + i2, (Object)Bukkit.getWorld(s).getBlockAt(l, m, i2).getState().getBlockData().getAsString().replaceFirst("minecraft:", ""));
                        }
                    }
                }
            }
